        ...
        void Update()
        {
...
#endif
            {
                if (!pressing && Input.GetMouseButtonDown(0))   //When pressed (left click / touch can be acquired).    //押したとき（左クリック/タッチが取得できる）
                {
                    ...
                    if (validArea.xMin * UnityEngine.Screen.width <= startPos.x && startPos.x <= validArea.xMax * UnityEngine.Screen.width &&
                        validArea.yMin * UnityEngine.Screen.height <= startPos.y && startPos.y <= validArea.yMax * UnityEngine.Screen.height)   //Within recognition area   //認識エリア内
                    {
                        pressing = true;
                ...
                else if (pressing && Input.GetMouseButtonUp(0))  //Only when it is already pressed (Note that this function can not distinguish which finger when touching 2 or more).   //既に押されているときのみ（※この関数は２つ以上タッチの場合、どの指か判別できないので注意）
                {
                    ...
                    if (Time.time < limitTime)  //Recognize before time limit   //時間制限前なら認識
                    {
                        ...
                        float requiredPx = widthReference ? UnityEngine.Screen.width * validWidth : UnityEngine.Screen.height * validWidth;
