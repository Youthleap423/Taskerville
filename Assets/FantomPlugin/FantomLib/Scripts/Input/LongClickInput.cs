        ...
        void Update()
        {
...
#endif
            {
                if (!pressing && Input.GetMouseButtonDown(0))   //When pressed (left click / touch can be acquired).    //押したとき（左クリック/タッチが取得できる）
                {
                    ...
                    minPos.Set(validArea.xMin * UnityEngine.Screen.width, validArea.yMin * UnityEngine.Screen.height);
                    maxPos.Set(validArea.xMax * UnityEngine.Screen.width, validArea.yMax * UnityEngine.Screen.height);
