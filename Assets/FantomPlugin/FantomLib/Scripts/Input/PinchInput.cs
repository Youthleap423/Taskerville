        ...
        void Update()
        {
            ...
            if (Input.touchCount == 2) //Operation with pinch (2 fingers only)  //ピンチでの操作（2本指のみ）
            {
                ...
                if (!IsPinching && touch.phase == TouchPhase.Began)   //Only when newly recognized  //新しく認識したときのみ
                {
                    ...
                    if (validArea.xMin * UnityEngine.Screen.width <= center.x && center.x <= validArea.xMax * UnityEngine.Screen.width && 
                        validArea.yMin * UnityEngine.Screen.height <= center.y && center.y <= validArea.yMax * UnityEngine.Screen.height)
                    {
                        ...
                        if (isNormalized)
                        {
                            float unit = widthReference ? UnityEngine.Screen.width : UnityEngine.Screen.height;
