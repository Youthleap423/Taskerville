            ...
            if (angleOperation.dragEnable || heightOperation.dragEnable)
            {
                ...
                if (!dragging && Input.GetMouseButtonDown(0))
                {
                    ...
                    if (validArea.xMin * UnityEngine.Screen.width <= startPos.x && startPos.x <= validArea.xMax * UnityEngine.Screen.width &&
                        validArea.yMin * UnityEngine.Screen.height <= startPos.y && startPos.y <= validArea.yMax * UnityEngine.Screen.height)
                    {
                        dragging = true;
                    }
                }
                else if (dragging)
                {
                    if (Input.GetMouseButton(0))
                    {
                        ...
                        {
                            float limit = (widthReference ? UnityEngine.Screen.width : UnityEngine.Screen.height) * angleOperation.dragWidthLimit;
