...
namespace SA.Android.Editor
{
    class AN_AppFeaturesUI : AN_ServiceSettingsUI
    {
        public override void OnAwake()
        {
            ...
            AddFeatureUrl("Dark Mode", "https://github.com/StansAssets/com.stansassets.android-native/wiki/Dark-Mode-(Theme)");
        ...
        protected override string Description => "Contains high-level classes encapsulating the overall Android application model.";

        protected override Texture2D Icon => AN_Skin.GetIcon("android_app.png");

        protected override SA_iAPIResolver Resolver => AN_Preprocessor.GetResolver<AN_CoreResolver>();
        ...
        protected override void OnServiceUI()
        {
            using (new SA_WindowBlockWithSpace(new GUIContent("Runtime Permissions")))
            {
                ...
                AN_Settings.Instance.SkipPermissionsDialog = IMGUILayout.ToggleFiled("Startup Permissions Dialog", AN_Settings.Instance.SkipPermissionsDialog, IMGUIToggleStyle.ToggleType.YesNo);
            }
            ...
            using (new SA_WindowBlockWithSpace(new GUIContent("Media Player")))
            {
                ...
                AN_Settings.Instance.MediaPlayer = IMGUILayout.ToggleFiled("API State", AN_Settings.Instance.MediaPlayer, IMGUIToggleStyle.ToggleType.EnabledDisabled);
            }
        }
    }
}
