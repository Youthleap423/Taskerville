...
namespace SA.Android.Editor
{
    class AN_VendingFeaturesUI : AN_ServiceSettingsUI
    {
        ...
        protected override string Description => "A service provided by Google Play that lets you sell digital content from inside an Android app or “in-app.”";

        protected override Texture2D Icon => AN_Skin.GetIcon("android_vending.png");

        protected override SA_iAPIResolver Resolver => AN_Preprocessor.GetResolver<AN_VendingResolver>();
        ...
        protected override void OnServiceUI()
        {
            ...
            using (new SA_WindowBlockWithSpace(new GUIContent("Licensing ")))
            {
                ...
                AN_Settings.Instance.Licensing = IMGUILayout.ToggleFiled("API Status", AN_Settings.Instance.Licensing, IMGUIToggleStyle.ToggleType.EnabledDisabled);
            ...
            using (new SA_WindowBlockWithSpace(new GUIContent("Google Play Billing ")))
            {
                ...
                AN_Settings.Instance.GooglePlayBilling = IMGUILayout.ToggleFiled("API Status", AN_Settings.Instance.GooglePlayBilling, IMGUIToggleStyle.ToggleType.EnabledDisabled);
        ...
        {
            product.Sku = IMGUILayout.TextField(ProductIdDLabel, product.Sku);
            product.Title = IMGUILayout.TextField(DisplayNameLabel, product.Title);
            product.Type = (AN_BillingClient.SkuType)IMGUILayout.EnumPopup(ProductType, product.Type);
            product.IsConsumable = IMGUILayout.ToggleFiled(ConsumableLabel, product.IsConsumable, IMGUIToggleStyle.ToggleType.YesNo);
            product.Price = IMGUILayout.TextField(PriceLabel, product.Price);
