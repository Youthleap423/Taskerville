...
namespace SA.Android.Editor
{
    class AN_LocalNotificationsFeaturesUI : AN_ServiceSettingsUI
    {
        ...
        protected override string Description => "Allows you to display & manage local notifications on the android device.";

        protected override Texture2D Icon => AN_Skin.GetIcon("android_notifications.png");

        protected override SA_iAPIResolver Resolver => AN_Preprocessor.GetResolver<AN_LocalNotificationsResolver>();
        ...
        protected override void OnServiceUI()
        {
            using (new SA_WindowBlockWithSpace(new GUIContent("Customization")))
            {
                ...
                ValidateAssets(AN_EditorSettings.Instance.NotificationIcons, AN_Settings.AndroidDrawablePath, k_RequiredIconExtension);
                ...
                ValidateAssets(AN_EditorSettings.Instance.NotificationAlertSounds, AN_Settings.AndroidRawPath, k_RequiredSoundExtension);
            }
        }
        ...
        Object DrawIconField(Rect position, Object asset)
        {
            ...
            if (asset != null)
            {
                ...
                if (!SA_AssetDatabase.IsAssetInsideFolder(asset, AN_Settings.AndroidDrawablePath))
                    GUI.color = Color.red;
            }
        ...
        Object DrawSoundField(Rect position, Object asset)
        {
            ...
            if (asset != null)
            {
                ...
                if (!SA_AssetDatabase.IsAssetInsideFolder(asset, AN_Settings.AndroidRawPath))
                    GUI.color = Color.red;
            }
        ...
        void DrawEmptyIcons()
        {
            EditorGUILayout.LabelField("Add icons you want to use as custom notification icons. The application icon will be used by default", PluginsEditorSkin.MiniLabelWordWrap);
        }
